...
$sort  = (isset($_REQUEST['sort'])  && in_array(trim(strtolower($_REQUEST['sort'])), array('goods_id', 'shop_price', 'last_update' , 'click_count', 'sales_count'))) ? trim($_REQUEST['sort'])  : $default_sort_order_type;//by Leah
...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'asynclist')
{
    ...
    $goods_list = category_get_goods2($children, $brand, $price_min, $price_max, $ext, $amount, $last, $sort, $order);
		foreach($goods_list as $key=>$var) 
		{
			 $sayList[] = array(				
				'pro-inner'=>'<div class="proImg-wrap"><a href="'.$var['url'].'"><img alt="'.$var['name'].'" src="'.$var['goods_img'].'"></a></div><div class="proInfo-wrap"><div class="proTitle"><a href="'.$var['url'].'">'.$var['name'].'</a></div><div class="proSKU"></div><div class="proPrice"><em>'.$var['price_str'].'</em></div><div class="proService"><del>'.$var['market_price'].'</del></div><div class="proSales">月销:<em>'.$var['sales_count'].'</em></div><div class="proIcons">'.$var['watermark_str'].'</div></div>'
				);
		}
...
$cache_id = sprintf('%X', crc32($cat_id . '-' . $display . '-' . $sort  .'-' . $order  .'-' . $page . '-' . $size . '-' . $_SESSION['user_rank'] . '-' .
    $_CFG['lang'] .'-'. $brand. '-' . $price_max . '-' .$price_min . '-' . $filter_attr_str)).time();
...
function category_get_goods2($children, $brand, $min, $max, $ext, $size, $last, $sort, $order)
{
    $display = $GLOBALS['display'];
    $where = "g.is_on_sale = 1 AND g.is_alone_sale = 1 AND ".
            "g.is_delete = 0 AND ($children OR " . get_extension_goods($children) . ')';
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
            ...
            'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' .
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
            "WHERE $where $ext ORDER BY $sort $order";
    $res = $GLOBALS['db']->selectLimit($sql, $size, $last);
    $arr = array();
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        if ($row['promote_price'] > 0)
        ...
        $arr[$row['goods_id']]['sales_count']      = get_goods_sales_count($row['goods_id']);
        $arr[$row['goods_id']]['watermark_str']    =  ($watermark_img != '') ? '<img width="55" height="16" src="themes/default/images/'.$watermark_img.'.png" alt="'.$row['goods_name'].'">' : '';;
        $arr[$row['goods_id']]['price_str']    =  ($promote_price > 0) ? price_format($promote_price) : price_format($row['shop_price']);
    }
