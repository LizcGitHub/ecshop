...
elseif ($_REQUEST['act'] == 'post')
{
    ...
    $arr = array();
    $sql = 'SELECT id, value FROM ' . $ecs->table('shop_config_wap');
    $res= $db->query($sql);
    ...
    foreach ($_POST['value'] AS $key => $val)
    {
        if($arr[$key] != $val)
        {
            $sql = "UPDATE " . $ecs->table('shop_config_wap') . " SET value = '" . trim($val) . "' WHERE id = '" . $key . "'";
            $db->query($sql);
        }
    }
    ...
    /* 处理上传文件 */
    $file_var_list = array();
    $sql = "SELECT * FROM " . $ecs->table('shop_config_wap') . " WHERE parent_id > 0 AND type = 'file'";
    $res = $db->query($sql);
    while ($row = $db->fetchRow($res))
    {
    ...
    foreach ($_FILES AS $code => $file)
    {
        ...
        if ((isset($file['error']) && $file['error'] == 0) || (!isset($file['error']) && $file['tmp_name'] != 'none'))
        {
            ...
            else
            {
                ...
                {
                    $sql = "UPDATE " . $ecs->table('shop_config_wap') . " SET value = '$file_name' WHERE code = '$code'";
                    $db->query($sql);
                }
                else
                {
    ...
    if (!empty($_POST['invoice_rate']))
    {
        ...
        $sql = "UPDATE " . $ecs->table('shop_config_wap') . " SET value = '" . serialize($invoice) . "' WHERE code = 'invoice_type'";
        $db->query($sql);
    }
...
function update_configure($key, $val='')
{
    if (!empty($key))
    {
        $sql = "UPDATE " . $GLOBALS['ecs']->table('shop_config_wap') . " SET value='$val' WHERE code='$key'";
...
function get_settings($groups=null, $excludes=null)
{
    ...
    /* 取出全部数据：分组和变量 */
    $sql = "SELECT * FROM " . $ecs->table('shop_config_wap') .
