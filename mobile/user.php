...
/* 显示页面的action列表 */
$ui_arr = array('register', 'login', 'profile', 'order_list', 'order_detail', 'address_list', 'act_edit_address', 'collection_list',
'message_list', 'tag_list', 'get_password', 'reset_password', 'booking_list', 'add_booking', 'account_raply',
'account_deposit', 'account_log', 'account_detail', 'act_account', 'pay', 'default', 'bonus', 'group_buy', 'group_buy_detail', 'affiliate', 'comment_list','validate_email','track_packages', 'transform_points','qpassword_name', 'get_passwd_question', 'check_answer', 'order_tracking');
...
if (in_array($action, $ui_arr))
{
    ...
    $sql = "SELECT value FROM " . $ecs->table('shop_config_wap') . " WHERE id = 419";
    $row = $db->getRow($sql);
    $car_off = $row['value'];
...
elseif ($action == 'order_tracking')
{
  ...
  $json  = new JSON;
	...
	if($order['shipping_status']==1){
		...
		include_once(ROOT_PATH . 'plugins/kuaidi100/kuaidi100_config.php');
		
		if(!empty($postcom)&&!empty($getNu))
		{
			$url1 = 'http://www.kuaidi100.com/applyurl?key='.$kuaidi100key.'&com='.$postcom.'&nu='.$getNu;
			$url2 = 'http://api.kuaidi100.com/api?id='.$kuaidi100key.'&com='.$postcom.'&nu='.$getNu.'&show=0&muti=1&order=desc';
		
			$powered = '<div style="display:none">查询服务由：<a href="http://www.kuaidi100.com" target="_blank" style="color:blue">快递100</a> 网站提供</div>';
			
			$get_content1 = get_url_contents($url1);
			$get_content2 = get_url_contents($url2);
			
			$get_content1 = '<iframe src="'.$get_content1.'" width="534" height="340" frameborder="no" border="0" marginwidth="0" marginheight="0" scrolling="no" allowtransparency="yes" style="display:none"><br/>' . $powered;
		}else {
			$result['status'] = 3 ;
			$result['message'] = '快递或物流单号不正确，查询失败' ;
			$get_content2 = $json->encode($result);
		}
	} else {
		$result['status'] = 4 ;
		$result['message'] = $_LANG['os'][$order['order_status']]."，".$_LANG['ps'][$order['pay_status']]."，".$_LANG['ss'][$order['shipping_status']]."，请等等再查" ;
		$get_content2 = $json->encode($result);
	}

  $smarty->assign('get_content1',  $get_content1);
  $smarty->assign('get_content2',  $get_content2);
	$smarty->display('user_transaction.dwt');
}
...
elseif ($action == 'async_order_list')
{
    ...
    $goods_list = get_user_orders($user_id, $amount, $last);
		
		foreach($goods_list as $key=>$var) 
		{
			 $sayList[] = array(
				'order_status'=>$var['order_status'],
				'order_content'=>'<a href="user.php?act=order_detail&amp;order_id='.$var['order_id'].'"><table width="100%" cellspacing="0" cellpadding="5" border="0" class="ectouch_table_no_border"><tbody><tr><td><img width="50" height="50" src="'.$var['order_thumb'].'"></td><td>订单编号：'.$var['order_sn'].'<br>订单金额：'.$var['total_fee'].'<br>下单时间：'.$var['order_time'].'</td><td style="position:relative"><span class="new-arr"></span></td></tr></tbody></table></a>'.$var['handler'],
				'order_tracking'=>'<a class="c-btn5" href="user.php?act=order_tracking&amp;order_id='.$var['order_id'].'">查看物流</a>',
				
				);
		}
		die($json->encode($sayList));
}
...
elseif ($action == 'address_list')
{
    ...
    $consignee_list = get_consignee_list($_SESSION['user_id']);
		$consignee_count=count($consignee_list);
		
		$smarty->assign('consignee_count',       $consignee_count);
		
    if ($consignee_count < 5 && $_SESSION['user_id'] > 0)
    {
...
elseif ($action == 'act_edit_address')
{
    ...
    $smarty->assign('lang', $_LANG);
		if($flag == 'display'){
			if($address_id){
				$consignee  = $db->getRow("SELECT * FROM " .$ecs->table('user_address'). " WHERE address_id='$address_id'");
...
elseif ($action == 'async_collection_list')
{
    ...
    $goods_list = get_collection_goods($user_id, $amount, $last);
		
		foreach($goods_list as $key=>$var) 
		{
			 $sayList[] = array(
				'collection'=>'<a href="'.$var['url'].'"><table width="100%" cellspacing="0" cellpadding="5" border="0" class="ectouch_table_no_border"><tbody><tr><td><img width="50" height="50" src="'.$var['thumb'].'"></td><td>'.$var['goods_name'].'<br>本店售价：'.$var['shop_price'].'</td><td align="right"><a style="color:#1CA2E1" href="'.$var['url'].'">加入购物车</a><br><a style="color:#1CA2E1" href="javascript:if (confirm(\''.$_LANG['remove_collection_confirm'].'\')) location.href=\'user.php?act=delete_collection&amp;collection_id='.$var['rec_id'].'\'">'.$_LANG['drop'].'</a></td></tr></tbody></table></a>'
				);
		}
		die($json->encode($sayList));
}
...
elseif ($action == 'async_message_list')
{
    ...
    $goods_list = get_message_list($user_id, $_SESSION['user_name'], $amount, $last, $order_id);
		
		foreach($goods_list as $key=>$var) 
		{
			 $sayList[] = array(
				'message'=>'<table width="100%" cellspacing="0" cellpadding="5" border="0" class="ectouch_table_no_border"><tbody><tr><td><span style="float:right"><a style="color:#1CA2E1" onclick="if (!confirm(\''.$_LANG['confirm_remove_msg'].'\')) return false;" href="user.php?act=del_msg&amp;id=3123&amp;order_id=0">'.$_LANG['drop'].'</a></span>'.$var['msg_type'].'：'.$var['msg_title'].' '.$var['msg_time'].' </td></tr><tr><td>'.$var['msg_content'].'</td></tr></tbody></table>'
				);
		}
		die($json->encode($sayList));
}
...
elseif ($action == 'async_comment_list')
{
    ...
    $goods_list = get_comment_list($user_id, $amount, $last);
		
		foreach($goods_list as $key=>$var) 
		{
			 $sayList[] = array(
				'comment'=>'<table width="100%" cellspacing="0" cellpadding="5" border="0" class="ectouch_table_no_border"><tbody><tr><td><span style="float:right"><a style="color:#1CA2E1" onclick="if (!confirm(\''.$_LANG['confirm_remove_msg'].'\')) return false;" href="user.php?act=del_cmt&amp;id='.$var['comment_id'].'">删除</a></span>评论：'.$var['cmt_name'].' - '.$var['formated_add_time'].' </td></tr><tr><td>'.$var['content'].'</td></tr></tbody></table>'
				);
		}
		die($json->encode($sayList));
}
