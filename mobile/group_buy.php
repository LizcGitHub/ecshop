...
//    var_dump("团购活动的数量:".$count);
    if ($count > 0)
    {
    	$goods_list = group_buy_list2($amount, $last);
			...
			{
				 $sayList[] = array(				
					'pro-inner'=>'<div class="proImg-wrap"><a href="'.$var['url'].'"><img alt="'.$var['goods_name'].'" src="../'.$var['goods_thumb'].'"></a></div><div class="proInfo-wrap"><div class="proTitle"><a href="'.$var['url'].'">'.$var['goods_name'].'</a></div><div class="proSKU"></div><div class="proPrice"><em>'.$var['price_ladder'][0]['formated_price'].'</em></div><div class="proService"><del>'.$var['market_price'].'</del></div><div class="proIcons">'.$var['watermark_str'].'</div></div>'
					);
			}
...
function group_buy_list2($size, $last)
{
    /* 取得团购活动 */
    $gb_list = array();
    $now = gmtime();
