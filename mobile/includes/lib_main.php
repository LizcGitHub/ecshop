...
{
    $sql = 'SELECT id, number, type FROM ' . $GLOBALS['ecs']->table('template_wap') .
                        ...
                        $keywords = '', $brand = 0, $price_min = 0, $price_max = 0, $display_type = 'list', $filter_attr='', $url_format='', $sch_array='')
{
    ...
    else
    {
        ...
        {
            $pager['page_first'] = ( $page > 1) ? build_uri($app, $uri_args, '', 1, $keywords) : ''; //by Leah
...
//            $pager['page_last']  = ($_to < $page_count) ? build_uri($app, $uri_args, '', $page_count, $keywords) : '';
            $pager['page_last']  = ($page != $page_count) ? build_uri($app, $uri_args, '', $page_count, $keywords) : '';
            $pager['page_kbd']  = ($_pagenum < $page_count) ? true : false;
            $pager['page_number'] = array();
            for ($i=$_from;$i<=$_to;++$i)
            {
...
function get_dyna_libs($theme, $tmp)
{
    ...
    $sql = 'SELECT region, library, sort_order, id, number, type' .
            ' FROM ' . $GLOBALS['ecs']->table('template_wap') .
...
function get_library_number($library, $template = null)
{
    ...
    if (!isset($lib_list[$template]))
    {
        ...
        $sql = "SELECT library, number FROM " . $GLOBALS['ecs']->table('template_wap') .
...
{
    $sql = 'SELECT * FROM '. $GLOBALS['ecs']->table('nav_wap') . '
